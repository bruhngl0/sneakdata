...
// continue without the database, since thats where all the data is stored
// Similarly, the clients of the DB conventionally panic
// using [db.Panic] is the database returns the corrupted data
// Code using multiple parallel database operations can recover
// at the outermost calls.

type DB interface {
	//Lock aquires a lock on the give name, which need not to exist in the database
	// After a successful Lock(name),
	// any other call to Lock(name) from any other client of the database,
	// (including in another processes, including shared databases)
	// must be blocked unless Unlock(name), is called.
	// In the shared databases, a lock may also Unlock,
	...
	// They may not be of length zero
	Set(key, val []byte)
	...
	//
	// A scan may or not observe concurrent modifications made
	...
	// DeleteRange deletes all the key value pairs with  start ≤ key ≤ end.
	...
	// Batch for bulk operations is more effecient than making each
	// change using repeateed calls to DB's Set, Delete, and DeleteRange methods.
	Batch() Batch

	// Flush flushes DB changes to a permanent storage.
	// Flush must be called before the process crashes or exists,
	// or else any changes since the previous Flush may be lost.
	Flush()

	// Close flushes and then closes the database
	// Like any other go routines, it panics if the error happens
	...
	// It is meant to be called when database corruption or any other
	// database-related "cant happen" conditions have been detected
...
// is also more effecient than making individual [DB] method calls.
